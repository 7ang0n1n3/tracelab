...
export async function testsRoutes(app: FastifyInstance) {
  ...
  app.post("/api/tests", { preHandler: requireAuth }, async (req, reply) => {
    ...
    const id = uuidv4();
    db.prepare(`
      INSERT INTO tests (id, user_id, name, description, app_name, base_url, script, tags, auth_state_id, use_auth, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      req.user!.id,
  ...
  app.put("/api/tests/:id", { preHandler: requireAuth }, async (req, reply) => {
    ...
    db.prepare(`
      UPDATE tests SET
        ...
        script = ?, tags = ?, auth_state_id = ?, use_auth = ?, updated_at = ?
      WHERE id = ?
    `).run(
